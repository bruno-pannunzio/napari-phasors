...
class LifetimeWidget(QWidget):
    ...
    def calculate_lifetimes(self):
        ...
        if self.lifetime_type_combobox.currentText() == "Phase":
            phase_lifetime = np.clip(phase_lifetime, a_min=0, a_max=None)
            self.lifetime_data_original = phase_lifetime
        else:
            modulation_lifetime = np.clip(
                modulation_lifetime, a_min=0, a_max=None
            )
            self.lifetime_data_original = modulation_lifetime
    ...
    def _on_click(self):
        ...
        update_frequency_in_metadata(
            self.parent_widget, self.frequency_input.text().strip()
        )
