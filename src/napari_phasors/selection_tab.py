...
class SelectionWidget(QWidget):
    ...
    def _ensure_selection_storage(self, selection_id: str):
        ...
        if selection_id not in layer.metadata["selections"]:
            ...
            # We use G array to determine shape
            if self.parent_widget._g_array is None:
                return
                
            g_shape = self.parent_widget._g_array.shape
            # Handle (Harmonics, Y, X) vs (Y, X)
            if len(g_shape) == 3:
                spatial_shape = g_shape[1:]
            else:
                spatial_shape = g_shape
        ...
        if self._current_selection_id != new_selection_id_for_comparison:
            ...
            if (
                ...
                if (
                    ...
                    and new_selection_id_for_comparison in layer.metadata["selections"]
                ):
    ...
    def update_phasor_plot_with_selection_id(self, selection_id):
        ...
        # Reconstruct mask (same logic as plotter.get_features)
        if self.parent_widget._g_array is None:
            return
            
        # Find index of current harmonic
        try:
            harmonic_idx = np.where(self.parent_widget._harmonics_array == self.parent_widget.harmonic)[0][0]
        except (IndexError, TypeError):
            return

        if self.parent_widget._g_array.ndim == 3:
            g_flat = self.parent_widget._g_array[harmonic_idx].ravel()
            s_flat = self.parent_widget._s_array[harmonic_idx].ravel()
        else:
            g_flat = self.parent_widget._g_array.ravel()
            s_flat = self.parent_widget._s_array.ravel()

        mask = np.isnan(g_flat) | np.isnan(s_flat)
        
        # Extract selection data for valid pixels
        selection_data = selection_map.ravel()[~mask]
    ...
    def manual_selection_changed(self, manual_selection):
        ...
        # Reconstruct mask to map plot indices back to image pixels
        if self.parent_widget._g_array is None:
            return
        ...
        valid_pixels_mask = ~(np.isnan(g_flat) | np.isnan(s_flat))
    ...
    def create_phasors_selected_layer(self):
        ...
        if "selections" not in layer.metadata or self.selection_id not in layer.metadata["selections"]:
            return
        ...
        layer_name = (
            f"Selection {self.selection_id}: {layer.name}"
        )
    ...
    def update_phasors_layer(self):
        ...
        if self.selection_id is None or self.selection_id == "":
            ...
            existing_phasors_selected_layer.data = np.zeros_like(existing_phasors_selected_layer.data)
        else:
            if "selections" in layer.metadata and self.selection_id in layer.metadata["selections"]:
                existing_phasors_selected_layer.data = layer.metadata["selections"][self.selection_id]
